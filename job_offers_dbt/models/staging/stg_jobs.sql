...
renamed_and_cast AS (
    ...
    SELECT
        ...
        CASE
            ...
            WHEN detected_extensions ->> 'posted_at' IS NULL 
            OR detected_extensions ->> 'posted_at' = '' THEN created_at

            -- 2. Handle "X hours ago" (heures)
            WHEN detected_extensions ->> 'posted_at' ~ 'il y a \d+ heure' THEN
                created_at - (
                    (REGEXP_MATCH(detected_extensions ->> 'posted_at', 'il y a (\d+) heure'))[1] || ' hours'
                )::INTERVAL

            -- 3. Handle "X days ago" (jours)
            WHEN detected_extensions ->> 'posted_at' ~ 'il y a \d+ jour' THEN
                created_at - (
                    (REGEXP_MATCH(detected_extensions ->> 'posted_at', 'il y a (\d+) jour'))[1] || ' days'
                )::INTERVAL

            -- 4. Handle "X minutes ago" (minutes) - just in case
            WHEN detected_extensions ->> 'posted_at' ~ 'il y a \d+ minute' THEN
                created_at - (
                    (REGEXP_MATCH(detected_extensions ->> 'posted_at', 'il y a (\d+) minute'))[1] || ' minutes'
                )::INTERVAL

            -- 5. Handle "X months ago" (mois)
            WHEN detected_extensions ->> 'posted_at' ~ 'il y a \d+ mois' THEN
                created_at - (
                    (REGEXP_MATCH(detected_extensions ->> 'posted_at', 'il y a (\d+) mois'))[1] || ' months'
                )::INTERVAL
