...
def load_data(path):
    df = pd.read_csv(path)
    skill_columns = ['outils_bi', 'langages', 'cloud', 'modelisation', 'work_titles', 'type_contrat']
    
    def convert_string_to_list(val):
        if isinstance(val, list): return val
        if pd.isna(val) or not isinstance(val, str) or not val.startswith('['): return []
        try:
    ...
    # On crée la colonne 'salaire_present' pour le camembert
...
try:
    source_df = load_data('offres_emploi_data_enriched_with_company_info.csv')
...
# --- NOUVEAUTÉ : Navigation entre les pages ---
...
# Page 2 : Décomposition des offres
    ...
    plot_value_counts_plotly(df_display, 'company_name', top_n=20, title="Top des entreprises")
